...
<?php
	...
	if(array_key_exists("paynow", $_POST)) {
        ...
        if (!$_GET['regid']||empty($_GET['regid'])) { 
        ...
        if ($error != "") {
        	...
        	$query = "SELECT * FROM `users` WHERE regid = '".mysqli_real_escape_string($link, $_GET['regid'])."' LIMIT 1";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    $row = $result->fetch_assoc();
                        if($row['verified']==0){
                            ...
                            header("Location: instamojo.php?regid=".$_GET['regid']."&name=".$row['name']."&email=".$row['email']."&phone=".$row['phone']."&kiitian=".$row['university']."&noofevents=".$noofevents);
                        }
                    ...
                    if(mysqli_num_rows($result) == 0){
                    	...
                    	//header("Location: instamojo.php?regid=$_GET['regid']");
                    }
